...
//            endPosition[i] = new Vector3(r * Mathf.Sin(i * 2 * Mathf.PI / numSphere), r * Mathf.Cos(i * 2 * Mathf.PI / numSphere));
            ...
            endPosition[i] = 5.0f * new Vector3(Mathf.Sqrt(2f) * sin * sin * sin, 
                    (-1 * cos * cos * cos) - (cos * cos) + (2 * cos), 0f);
        }
    ...
    void Update()
    {
        ...
        // Scale time to move faster and slower according to normalized sin
        float time_scale = Mathf.Lerp(0.5f, 5.0f, Mathf.Sin(Time.time * (2f * Mathf.PI / 8.0f)) * 0.5f + 0.5f); 
        time += Time.deltaTime * time_scale; // Time.deltaTime = The interval in seconds from the last frame to the current one

        // Adjust image alpha
        float alpha = maxAlpha * Mathf.Pow(1f - (Mathf.Sin(Time.time * (2f * Mathf.PI / 8.0f)) * 0.5f + 0.5f), 3.0f);
        Color overlayColor = shadow.color;
        overlayColor.a = alpha;
        shadow.color = overlayColor; 
        ...
        for (int i =0; i < numSphere; i++){
            ...
            Renderer sphereRenderer = spheres[i].GetComponent<Renderer>();
            float hue = (float)i / numSphere; // Hue cycles through 0 to 1
            float control = Mathf.Repeat(hue + time * 0.67f, 1f);
